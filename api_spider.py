...
class Web_api:
    ...
    def judge_is_api(self, regex, cont):
        for line, code_str in enumerate(cont):
            ...
            if r:
                return r[0], r.end(), True, line    # 这里的line是从0开始的
    ...
    def run(self):
        ...
        regex_js = re.compile(r"\bapi\.|(?<!\w)\$\.ajax(?!\w)")    # 将正则对象提前准备好，优化时间
                                ...
                                for cont in diff_data["content"]:
                                    if "ab" in cont:    # 公共代码, 记录代码行号，同时记录
                                        ...
                                        if len(temp_save) > 0:
                                            ...
                                            temp_save_two = self.judge_code(temp, cont["b"], code_type, regex_js, regex_php)
